...
- the name of the new column where those old column names (actually variable values) will be stored (in this case `direction`)

- the name of the new column where the values in the problem columns will be stored (in this case, `duration`).
...
The `tidyr` package, which loads with `tidyverse`, includes sevaral version of the same data set. `table1` is the tidy version.
...
Here, two variables are contained in a single column. Use `separate` on this (not covered in this class). 
